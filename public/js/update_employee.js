...
updateEmployeeForm.addEventListener("submit", function (e) {
    ...
    let inputPayRate = document.getElementById("input-payRate-update");
...
function updateRow(data, idEmployee){
    ...
    for (let i = 0, row; row = table.rows[i]; i++) {
       ...
       if (table.rows[i].getAttribute("data-value") == idEmployee) {
            ...
            let payRatetd = updateRowIndex.getElementsByTagName("td")[1];
            let firstNametd = updateRowIndex.getElementsByTagName("td")[2];
            let lastNametd = updateRowIndex.getElementsByTagName("td")[3];
            let emailtd = updateRowIndex.getElementsByTagName("td")[4];
            let phonetd = updateRowIndex.getElementsByTagName("td")[5];
            let addresstd = updateRowIndex.getElementsByTagName("td")[6];

            // Reassign homeworld to our value we updated to
            payRatetd.innerHTML = parsedData[0].payRate; 
